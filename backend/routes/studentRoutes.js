...
router.post('/checkin', async (req, res) => {
  ...
  try {
    ...
    if (!student) return res.status(200).json({ success: false, message: 'Invaid Student ID' })
    else {
      const check = await Student.findOne({ studentId, outTime: null })
      if (check) return res.status(200).json({ success: false, message: `Please Checkout the previous visit for ${student.name} before the new visit` })
      else {
...
router.post('/checkout', async (req, res) => {
  ...
  try {
    ...
    if (!student) return res.status(200).json({ success: false, message: 'Invaid Student ID' })
      ...
      if (!record) return res.status(200).json({ success: false, message: 'No Entry time found for this student' })
      else {
